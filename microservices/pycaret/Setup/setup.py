...
class SetupPycaret:
    ...
    def create(self,
        ...
        logging.info('self.__metadata_creator.create_file') 
        self.__metadata_creator.create_file(self.model_name,
                                            self.service_type,
                                            self.module_path,
                                            self.class_name)
        logging.info('elf.__thread_pool.submit')
    ...
    def __pipeline(self,
        ...
        try:
            logging.info(f'Impostando modulos {self.module_path}')
            module = importlib.import_module(self.module_path)
            logging.info(f'Modulo importado {module}')
            class_reference = getattr(module, self.class_name)
            logging.info(f'Função referencia {class_reference}')
            class_instance = self.__create_a_class_instance(class_reference,
                                                            class_parameters)
            logging.info(f'Salvando configuração {class_reference}')                                                
            class_instance.save_config(f'{self.model_name}.pkl')                                                            
            logging.info(f'Salvando class_instance {class_reference}')                                                
            self.__storage.save(self.model_name,
                                class_instance,
                                self.service_type)
            logging.info(f'Atualizando flag finish')                      
    ...
    def __create_a_class_instance(self,
        ...
        result = class_reference(**treated_parameters)
        return result    